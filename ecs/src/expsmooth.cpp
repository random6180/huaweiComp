...
vector<double> expSmooth(vector<double>& original, double a) {
	vector<double> smooth(original.size() + 1, 0);
	smooth[0] = (original[0] + original[1] + original[2]) / 3;

	for (unsigned int i = 1; i < smooth.size() - 1; i++) {
		smooth[i] = a*original[i - 1] + (1 - a)*smooth[i - 1];
	}
	return smooth;
}


double preDay(vector<double>& onesmooth, vector<double>& twosmooth, int index, int pias = 1, double a = 0.9) {
	vector<double> A(onesmooth.size(), 0);
	vector<double> B(onesmooth.size(), 0);

	for (unsigned int i = 0; i < A.size(); i++) {
		A[i] = 2 * onesmooth[i] - twosmooth[i];
		B[i] = (a / (1 - a))*(onesmooth[i] - twosmooth[i]);
	}

	return A[index] + B[index] * pias;
}
...
//	int servernum = 0;
	...
	for (unsigned int i = 0; i < flavorlist.size(); i++) {									//对flavor对象的num赋值，即预测过程
		...
		vector<double> onesmooth;
		vector<double> twosmooth;
		double a = 0.3;

		onesmooth = expSmooth(sumflavorlist, a);
		twosmooth = expSmooth(onesmooth, a);

		for (int ui = 0; ui < days; ui++) {
			result = preDay(onesmooth, twosmooth, sumflavorlist.size() - 1, ui + 1, a);
	...
	flavortopre[0].num = 8;
	flavortopre[1].num = 49;
	flavortopre[2].num = 44;
	flavortopre[3].num = 11;
	flavortopre[4].num = 29;
	flavortopre[5].num = 2;
	flavortopre[6].num = 10;
	flavortopre[7].num = 17;
	flavortopre[8].num = 48;
	flavortopre[9].num = 42;
	flavortopre[10].num = 42;
	flavortopre[11].num = 12;
	flavortopre[12].num = 27;
	flavortopre[13].num = 12;
	flavortopre[14].num = 20;
	...
	serverlist = serverlist1;															//写入服务器的分配信息
	...
	for (unsigned int i = 0; i < serverlist.size(); i++) {					//打印分配信息
		int cpupro = 0;
		int memorypro = 0;
		cout << "Serverid" << serverlist[i].id+1 << ": ";
